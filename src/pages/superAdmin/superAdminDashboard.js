...
import {
  FaUserShield, FaCheckCircle, FaClock, FaLayerGroup,
  FaSignOutAlt, FaBell, FaTrash, FaEdit, FaTimes, FaPaperPlane,
} from "react-icons/fa";
...
export default function SuperAdminDashboard() {
  ...
  const [form, setForm] = useState({ subject: "", message: "", targetType: "admin" });
  ...
  const fetchNotifications = () => {
    axios.get("/notifications/all", { headers })
  ...
  const handleEdit = (n) => {
    setEditingId(n._id);
    setForm({ subject: n.subject, message: n.message, targetType: n.targetType });
    setShowPanel(true);
  ...
  const cards = [
    { label: "Total Admins",    value: stats.totalAdmins    ?? 0, color: "bg-blue-500",   icon: <FaUserShield size={24} /> },
    { label: "Approved",        value: stats.approvedAdmins ?? 0, color: "bg-green-500",  icon: <FaCheckCircle size={24} /> },
    { label: "Pending",         value: stats.pendingAdmins  ?? 0, color: "bg-yellow-500", icon: <FaClock size={24} /> },
    { label: "Remaining Slots", value: stats.remainingSlots ?? 0, color: "bg-purple-500", icon: <FaLayerGroup size={24} /> },
  ];
  ...
  const targetBadge = (t) => {
    ...
    return (
      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${map[t]}`}>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-gray-900 text-white px-8 py-4 flex items-center justify-between shadow sticky top-0 z-10">
        <h1 className="text-2xl font-bold tracking-wide">⚙️ Super Admin Panel</h1>
        <div className="flex items-center gap-3">
          <button
            onClick={() => { setShowPanel(!showPanel); cancelEdit(); }}
      ...
      <main className="p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
          {cards.map((c) => (
            <div key={c.label} className={`${c.color} text-white rounded-xl p-5 shadow flex items-center gap-4`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          >
            <h2 className="text-xl font-bold text-gray-800 mb-1">🛡️ Admin Approval & Permissions</h2>
            <p className="text-gray-500 text-sm">Approve pending admins and control page access.</p>
          </div>

          <div
            onClick={() => { setShowPanel(true); cancelEdit(); }}
          ...
          >
            ...
            <p className="text-gray-500 text-sm">
              Send announcements to admins, vendors, or both. {notifications.length} sent so far.
            </p>
          </div>
        </div>
        ...
        {showPanel && (
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => { setShowPanel(false); cancelEdit(); }} className="text-gray-400 hover:text-gray-700">
                <FaTimes size={18} />
              </button>
            </div>
                  ...
                  {["admin", "vendor", "both"].map((t) => (
                    <label key={t} className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-sm capitalize font-medium text-gray-700">
                        {t === "both" ? "Both" : t === "admin" ? "Admins" : "Vendors"}
                      </span>
                    </label>
                  ))}
                </div>
            ...
            <div className="flex gap-3">
              ...
              >
                <FaPaperPlane /> {editingId ? "Update" : "Send Notification"}
              </button>
              {editingId && (
                <button
                  onClick={cancelEdit}
            ...
            {notifications.length > 0 && (
              <div className="mt-8">
                <h3 className="text-sm font-bold text-gray-700 mb-3 uppercase tracking-wide">Sent Notifications</h3>
                <div className="space-y-3">
                  {notifications.map((n) => (
                    <div key={n._id} className="flex items-start justify-between bg-gray-50 border border-gray-200 rounded-xl p-4">
