    ...
    <div className="max-w-3xl mx-auto bg-white p-4 sm:p-6 rounded shadow-md">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {Object.keys(formData)
          .filter(
            (field) =>
              !["_id", "images", "createdAt", "updatedAt", "__v", "status", "watermarkedImages"].includes(
                field
              )
          )
          .map((field) => (
        ...
        <div className="col-span-1 sm:col-span-2">
          <button
            type="submit"
            className="w-full sm:w-auto sm:ml-auto block bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700"
          >
            Update Product
          </button>
        </div>
